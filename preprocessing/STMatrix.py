...
class STMatrix(object):
    ...
    def check_complete(self):
        ...
        while i < len(pd_timestamps):
            if pd_timestamps[i - 1] + offset != pd_timestamps[i]:  #找到后30分钟有无数据
    ...
    def create_dataset(self, len_closeness=3, len_trend=3, TrendInterval=7, len_period=3, PeriodInterval=1):
        ...
        while i < len(self.pd_timestamps):
            ...
            if Flag is False:
                # print(self.pd_timestamps[i])
                # print([self.pd_timestamps[i] - j * offset_frame for j in depends[2]])
                i += 1
                continue
