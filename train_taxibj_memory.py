...
now=datetime.now()
save_path='reports/taxibj/memory/{date:%Y-%m-%d_%H %M %S}'.format(date=now)
#save_path='reports/taxibj/st_net/2022-03-21_14 07 49'.format(date=now)
out_dir = './reports/taxibj'
checkpoint_dir = save_path+'/checkpoint'
os.makedirs(checkpoint_dir, exist_ok=True)
writer = SummaryWriter('reports/taxibj/memory/{date:%Y-%m-%d_%H %M %S}/log'.format(date=now))


# initial_checkpoint = 'reports/taxibj/checkpoint/initial_00000100_model.pth'
LOAD_INITIAL = False
...
def train():
    ...
    fix_random_seeds()
    # data loader
    train_dataset = make_dataloader(dataset_name='taxibj', mode='train',
    ...
    model = mstnet1(learning_rate=learning_rate,
                     ...
                     external_dim = 28, nb_residual_unit = nb_residual_unit)
    # if LOAD_INITIAL:
    ...
    for e in range(epoch_nums):
        ...
        print("Mean valid  mae(real):%.6f  ,rmse(real):%.6f "%(mae_loss*(train_dataset.mmn._max-train_dataset.mmn._min)/2,rmse_loss*(train_dataset.mmn._max-train_dataset.mmn._min)/2))


        if es.step(mse_loss,e+1):
            print('early stopped! With val loss:', mse_loss)
            print('best_epoch:{}'.format(es.get_best_epoch()))
